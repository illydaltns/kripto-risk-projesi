...
def plot_residual_distribution(y_true, y_pred, model_name="Model"):
    ...
    if not np.all(mask):
        print(
            f"Uyarı: {model_name} residuals içinde {np.sum(~mask)} adet geçersiz (NaN/Inf) değer var. Bunlar temizleniyor."
        )
    ...
    print(
        f"{model_name} Residuals - Min: {residuals_clipped.min()}, Max: {residuals_clipped.max()}, Mean: {residuals_clipped.mean()}"
    )
...
def get_linear_regression_coefficients(model, feature_names):
    ...
    df = pd.DataFrame({"Özellik": feature_names, "Katsayı": coefs})
